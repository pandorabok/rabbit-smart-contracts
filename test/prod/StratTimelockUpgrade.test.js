...
const config = {
  ...
  timelock: addressBook[chainName].platforms.rabbityieldfinance.vaultOwner,
};
...
describe("StratUpgrade", function () {
  ...
  before(async () => {
    ...
    vault = await ethers.getContractAt("RabbityieldVaultV6", config.vault);
  ...
  it("New strat has the correct admin accounts", async () => {
    const { rabbityieldfinance } = addressBook[chainName].platforms;
    expect(await candidate.keeper()).to.equal(rabbityieldfinance.keeper);
    expect(await candidate.owner()).to.equal(rabbityieldfinance.strategyOwner);
  });
