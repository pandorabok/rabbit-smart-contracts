...
import { RabbityieldChain } from "../../utils/rabbityieldChain";

const {
  platforms: { spookyswap, spiritswap, rabbityieldfinance },
  tokens: {
    TUSD: { address: TUSD },
    FTM: {address: FTM},
    USDC: { address: USDC },
...
async function main() {
  ...
  await setCorrectCallFee(strategy, hardhat.network.name as RabbityieldChain);
  console.log();
