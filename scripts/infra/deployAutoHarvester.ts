...
import { RabbityieldHarvester, UpkeepRefunder } from "../../typechain-types";
...
const config = {
  harvester: {
    contractName: "RabbityieldHarvester",
    args: {
      vaultRegistry: chainData.platforms.rabbityieldfinance.vaultRegistry,
      keeperRegistry,
...
const deployHarvester = async (upkeepAddress: string) => {
  const RabbityieldHarvesterFactory = await ethers.getContractFactory(config.harvester.contractName);
