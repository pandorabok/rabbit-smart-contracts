...
contract RabbityieldFeeConfigurator is OwnableUpgradeable {
    ...
    function getFeeCategory(uint256 _id, bool _adjust) public view returns (FeeCategory memory fees) {
        ...
        if (_adjust) {
            ...
            fees.rabbityield = fees.rabbityield * _totalFee / DIVISOR;
    ...
    function setFeeCategory(
        ...
        uint256 rabbityield = DIVISOR - _call - _strategist;

        FeeCategory memory category = FeeCategory(_total, rabbityield, _call, _strategist, _label, _active);
        feeCategory[_id] = category;
        emit SetFeeCategory(_id, _total, rabbityield, _call, _strategist, _label, _active);
    }
