...
/// @title Reward pool for RABT
/// @author kexley, Rabbityield
/// @notice Multi-reward staking contract for RABT
/// @dev Multiple rewards can be added to this contract by the owner. A receipt token is issued for 
/// staking and is used for withdrawing the staked RABT.
contract RabbityieldRewardPool is ERC20Upgradeable, OwnableUpgradeable {
    ...
    /// @notice Withdraw RABT tokens
    /// @dev Burns an equal number of receipt tokens from the caller
    /// @param _amount Amount of RABT to withdraw
    function withdraw(uint256 _amount) external update(msg.sender) {
        _withdraw(_amount);
    }

    /// @notice Withdraw all of the caller's RABT tokens and claim rewards
    ...
    /// @dev Stake RABT tokens and mint the caller receipt tokens
    ...
    /// @dev Withdraw RABT tokens and burn an equal number of receipt tokens from the caller
    /// @param _amount Amount of RABT to withdraw
    ...
    /// @dev Calculate the reward amount per RABT token
    /// @param _reward Address of the reward
    /// @return rewardPerToken Reward amount per RABT token
