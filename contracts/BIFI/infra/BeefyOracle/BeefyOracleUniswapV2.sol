...
/// @title Rabbityield Oracle for UniswapV2
...
contract RabbityieldOracleUniswapV2 {
    ...
    function validateData(bytes calldata _data) external view {
        ...
        for (uint i; i < pairLength;) {
            ...
            if (fromToken != token0 && fromToken != token1) {
                revert RabbityieldOracleErrors.TokenNotInPair(fromToken, pair);
            }
            if (toToken != token0 && toToken != token1) {
                revert RabbityieldOracleErrors.TokenNotInPair(toToken, pair);
            }
            unchecked { ++i; }
        }
    }
