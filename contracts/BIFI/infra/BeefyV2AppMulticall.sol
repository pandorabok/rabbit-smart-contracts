...
interface IRabbityieldVault {
    function balance() external view returns (uint256);
    function getPricePerFullShare() external view returns (uint256);
    function strategy() external view returns (IRabbityieldStrategy);
}

interface IRabbityieldCowVault {
    function balances() external view returns (uint256, uint256);
    function strategy() external view returns (IRabbityieldStrategy);
}

interface IRabbityieldBoost {
...
interface IRabbityieldBoostV2 {
    function balanceOf(address) external view returns (uint256);
    function earned(address) external view returns (address[] memory, uint256[] memory);
}

interface IRabbityieldMultiRewardPool {
    function totalSupply() external view returns (uint256);
    function rewardsLength() external view returns (uint256);
    function earned(address) external view returns (address[] memory, uint256[] memory);
    function rewardInfo(uint256) external view returns (address, uint256, uint256, uint256, uint256);
}
...
contract RabbityieldV2AppMulticall {
    function getVaultInfo(address[] calldata vaults) external view returns (VaultInfo[] memory) {
        ...
        for (uint i = 0; i < vaults.length; i++) {
            IRabbityieldVault vault = IRabbityieldVault(vaults[i]);
            IRabbityieldStrategy strat = vault.strategy();
            bool paused;
    ...
    function getCowVaultInfo(address[] calldata vaults) external view returns (CowVaultInfo[] memory) {
        ...
        for (uint i = 0; i < vaults.length; i++) {
            IRabbityieldCowVault vault = IRabbityieldCowVault(vaults[i]);
            IRabbityieldStrategy strat = vault.strategy();
            bool paused;
            try strat.paused() returns (bool _paused) {
                paused = _paused;
            ...
            results[i] = CowVaultInfo(balance0, balance1, address(strat), paused);
        }
    ...
    function getBoostInfo(address[] calldata boosts) external view returns (BoostInfo[] memory) {
        ...
        for (uint i = 0; i < boosts.length; i++) {
            IRabbityieldBoost boost = IRabbityieldBoost(boosts[i]);
            ...
            results[i] = BoostInfo(boost.totalSupply(), boost.rewardRate(), periodFinish, isPreStake);
        }

        return results;
    }

    function getGovVaultMultiBalance(
        address[] calldata vaults,
        address owner
    ) external view returns (BoostBalanceInfoV2[] memory) {
        BoostBalanceInfoV2[] memory results = new BoostBalanceInfoV2[](vaults.length);
        for (uint i; i < vaults.length; i++) {
            IRabbityieldBoostV2 boost = IRabbityieldBoostV2(vaults[i]);
            (address[] memory rewardTokens, uint256[] memory rewards) = boost.earned(owner);
            results[i] = BoostBalanceInfoV2(boost.balanceOf(owner), rewardTokens, rewards);
        }
    ...
    function getGovVaultInfo(address[] calldata govVaults) external view returns (GovVaultInfo[] memory) {
        ...
        for (uint i = 0; i < govVaults.length; i++) {
            IRabbityieldBoost govVault = IRabbityieldBoost(govVaults[i]);
            results[i] = GovVaultInfo(govVault.totalSupply());
        }

        return results;
    }

    function getGovVaultMultiInfo(address[] calldata govVaults) external view returns (GovVaultMultiInfo[] memory) {
        GovVaultMultiInfo[] memory results = new GovVaultMultiInfo[](govVaults.length);
        for (uint i = 0; i < govVaults.length; i++) {
            IRabbityieldMultiRewardPool govVault = IRabbityieldMultiRewardPool(govVaults[i]);

            uint256 rewardCount;
            try govVault.rewardsLength() returns (uint256 _length) {
                rewardCount = _length;
            } catch {
            ...
            for (uint j = 0; j < rewardCount; j++) {
                ...
                rewards[j] = RewardInfo(rewardAddress, rate, periodFinish);
            }
            results[i] = GovVaultMultiInfo(govVault.totalSupply(), rewards);
        }
    ...
    function getBoostOrGovBalance(
        address[] calldata boosts,
        address owner
    ) external view returns (BoostBalanceInfo[] memory) {
        BoostBalanceInfo[] memory results = new BoostBalanceInfo[](boosts.length);
        for (uint i = 0; i < boosts.length; i++) {
            IRabbityieldBoost boost = IRabbityieldBoost(boosts[i]);
            results[i] = BoostBalanceInfo(boost.balanceOf(owner), boost.earned(owner));
        }

        return results;
    }

    function getGovVaultBalance(
        address[] calldata govVaults,
        address owner
    ) external view returns (GovVaultBalanceInfo[] memory) {
        GovVaultBalanceInfo[] memory results = new GovVaultBalanceInfo[](govVaults.length);
        for (uint i = 0; i < govVaults.length; i++) {
            IRabbityieldBoost govVault = IRabbityieldBoost(govVaults[i]);
            results[i] = GovVaultBalanceInfo(govVault.balanceOf(owner), govVault.earned(owner));
        }
    ...
    function getAllowances(
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ...
            results[i] = AllowanceInfo(new uint256[](tokenSpenders.length));
