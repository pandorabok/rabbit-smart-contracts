...
// Minimal proxy pattern for creating new Rabbityield strategies
contract StrategyFactory is Ownable {
    ...
    /// @notice The rabbityield fee recipient
    address public rabbityieldFeeRecipient;

    /// @notice The rabbityield fee config
    address public rabbityieldFeeConfig;
    ...
    event SetRabbityieldFeeRecipient(address rabbityieldFeeRecipient);
    event SetRabbityieldFeeConfig(address rabbityieldFeeConfig);
    ...
    constructor(
        ...
        address _rabbityieldFeeConfig
    ) Ownable(msg.sender) {
        native = _native;
        keeper = _keeper;
        rabbityieldFeeRecipient = _rabbityieldFeeRecipient;
        rabbityieldFeeConfig = _rabbityieldFeeConfig;
    }

    /** @notice Creates a new Rabbityield Strategy as a proxy of the template instance
    * @param _strategyName The name of the strategy
    * @return A reference to the new proxied Rabbityield Strategy
   */
    function createStrategy(string calldata _strategyName) external returns (address) {

        // Create a new Rabbityield Strategy as a proxy of the template instance
   ...
   */
    ...
    /**
     * @notice set the rabbityield fee recipient address
   * @param _rabbityieldFeeRecipient The new rabbityield fee recipient address
   */
    function setRabbityieldFeeRecipient(address _rabbityieldFeeRecipient) external onlyOwner {
        rabbityieldFeeRecipient = _rabbityieldFeeRecipient;
        emit SetRabbityieldFeeRecipient(_rabbityieldFeeRecipient);
    }

    /**
     * @notice set the rabbityield fee config address
   * @param _rabbityieldFeeConfig The new rabbityield fee config address
   */
    function setRabbityieldFeeConfig(address _rabbityieldFeeConfig) external onlyOwner {
        rabbityieldFeeConfig = _rabbityieldFeeConfig;
        emit SetRabbityieldFeeConfig(_rabbityieldFeeConfig);
    }
