...
import "../../interfaces/rabbityield/IStrategyFactory.sol";
...
abstract contract BaseAllToNativeFactoryStrat is OwnableUpgradeable, PausableUpgradeable {
    ...
    function _chargeFees(address callFeeRecipient) internal {
        IFeeConfig.FeeCategory memory fees = rabbityieldFeeConfig().getFees(address(this));
    ...
    function _swap(address tokenFrom, address tokenTo, uint amount) internal {
        if (tokenFrom != tokenTo) {
            ...
            IRabbityieldSwapper(swapper).swap(tokenFrom, tokenTo, amount);
        }
    }
    ...
    function rabbityieldFeeConfig() public view returns (IFeeConfig) {
        return IFeeConfig(factory.rabbityieldFeeConfig());
    }

    function rabbityieldFeeRecipient() public view returns (address) {
        return factory.rabbityieldFeeRecipient();
    }

    function getAllFees() external view returns (IFeeConfig.AllFees memory) {
        return IFeeConfig.AllFees(rabbityieldFeeConfig().getFees(address(this)), depositFee(), withdrawFee());
    }
