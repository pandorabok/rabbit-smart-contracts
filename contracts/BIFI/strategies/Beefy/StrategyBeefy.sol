...
import { IRabbityieldRewardPool } from "../../interfaces/rabbityield/IRabbityieldRewardPool.sol";
import { StratFeeManagerInitializable, IFeeConfig } from "../Common/StratFeeManagerInitializable.sol";

/// @title Strategy staking in the RABT reward pool
/// @author kexley, Rabbityield
/// @notice Strategy managing the rewards from the RABT reward pool
contract StrategyRabbityield is StratFeeManagerInitializable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    /// @notice RABT token address
    address public want;
    ...
    /// @notice Reward pool for RABT rewards
    ...
    /// @param rabbityieldFees Amount of native sent to the rabbityield fee recipient
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IRabbityieldRewardPool(rewardPool).stake(wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IRabbityieldRewardPool(rewardPool).withdraw(_amount - wantBal);
    ...
    function _harvest(address _callFeeRecipient) internal whenNotPaused {
        IRabbityieldRewardPool(rewardPool).getReward();
        _swapToNative();
    ...
    function _swapToNative() internal {
        for (uint i; i < rewards.length; ++i) {
            ...
            if (rewardBal > 0) IRabbityieldSwapper(unirouter).swap(reward, native, rewardBal);
        }
    }
    ...
    function _chargeFees(address _callFeeRecipient) internal {
        ...
        IERC20Upgradeable(native).safeTransfer(rabbityieldFeeRecipient, rabbityieldFeeAmount);
    ...
    function rewardsAvailable() public view returns (uint256 unclaimedReward) {
        unclaimedReward = IRabbityieldRewardPool(rewardPool).earned(address(this), native);
    }
    ...
    function retireStrat() external {
        ...
        IRabbityieldRewardPool(rewardPool).withdraw(balanceOfPool());
