...
contract StrategyGM is StratFeeManagerInitializable {
    ...
    function _swapRewardsToNative() internal {
        for (uint i; i < rewards.length; ++i) {
            ...
            if (reward != native) {
                ...
                if (amount > 0) {
                    IRabbityieldSwapper(unirouter).swap(reward, native, amount);
                }
            }
        }
    }
    ...
    function _mintGm() internal {
        ...
        if (long != native) IRabbityieldSwapper(unirouter).swap(native, long, halfAmount);
        if (short != native) IRabbityieldSwapper(unirouter).swap(native, short, nativeBal - halfAmount);
