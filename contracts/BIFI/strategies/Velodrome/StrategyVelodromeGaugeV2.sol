...
contract StrategyVelodromeGaugeV2 is StratFeeManagerInitializable {
    ...
    address public rabbityieldSwapper;
    ...
    event SetRabbityieldSwapper(address newSwapper);

    function initialize(
        address _want,
        address _gauge,
        address _rabbityieldSwapper,
        ...
        want = _want;
        gauge = _gauge;
        rabbityieldSwapper = _rabbityieldSwapper;
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        (address router,,,,) = IRabbityieldSwapper(rabbityieldSwapper).swapInfo(output, native);
        if (router != address(0)) {
            IRabbityieldSwapper(rabbityieldSwapper).swap(output, native, toNative);
        } else {
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            (address router,,,,) = IRabbityieldSwapper(rabbityieldSwapper).swapInfo(output, lpToken0);
            if (router != address(0)) {
                IRabbityieldSwapper(rabbityieldSwapper).swap(output, lpToken0, lp0Amt);
            } else {
        ...
        if (lpToken1 != output) {
            (address router,,,,) = IRabbityieldSwapper(rabbityieldSwapper).swapInfo(output, lpToken1);
            if (router != address(0)) {
                IRabbityieldSwapper(rabbityieldSwapper).swap(output, lpToken1, lp1Amt);
            } else {
    ...
    function _giveAllowances() internal {
        ...
        IERC20(output).safeApprove(rabbityieldSwapper, type(uint).max);
    ...
    function _removeAllowances() internal {
        ...
        IERC20(output).safeApprove(rabbityieldSwapper, 0);
    ...
    function setRabbityieldSwapper(address _rabbityieldSwapper) external onlyManager {
        IERC20(output).safeApprove(rabbityieldSwapper, 0);
        rabbityieldSwapper = _rabbityieldSwapper;
        if (!paused()) IERC20(output).safeApprove(rabbityieldSwapper, type(uint).max);
        emit SetRabbityieldSwapper(_rabbityieldSwapper);
    }
}