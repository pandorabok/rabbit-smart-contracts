...
contract RabbityieldFeeBatchV3UniV3 is Initializable, OwnableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IERC20Upgradeable public wNative;
    IERC20Upgradeable public rabt;
    ...
    bytes public wNativeToRabtRoute;
    bytes public wNativeToStableRoute;

    bool public splitTreasury;
    bool public sendHarvesterGas;

    event Harvest(uint256 totalHarvested, uint256 stablesHarvested, uint256 rabtHarvested, uint256 nativeNotified, uint256 time);
    ...
    event NewRabtRoute(bytes oldRoute, bytes newRoute);
    ...
    function harvest() public {
        ...
        if (splitTreasury) {
            ...
            rabtAmt = nativeAvailable - stableAmt;
            stableOut = UniV3Actions.swapV3WithDeadline(unirouter, wNativeToStableRoute, stableAmt, treasury);
            rabtOut = UniV3Actions.swapV3WithDeadline(unirouter, wNativeToRabtRoute, rabtAmt, treasury);
        } else {
        ...
        emit Harvest(wNativeBal, stableOut, rabtOut, rewardPoolAmount, block.timestamp);
    }
