...
contract RabbityieldFeeBatchV3 is Initializable, OwnableUpgradeable {
    ...
    function harvest() public {
        ...
        if (splitTreasury) {
            ...
            IUniswapRouter(unirouter).swapExactTokensForTokens(rabtAmt, 0, wNativeToRabtRoute, treasury, block.timestamp);
        } else {
        ...
        emit Harvest(wNativeBal, stableAmt, rabtAmt, rewardPoolAmount, block.timestamp);
    }
    ...
    function setRoute(address[] calldata _route, bool _stableRoute) external onlyOwner {
        ...
        if(_stableRoute) {
            ...
            require(_route[_route.length - 1] == address(rabt), "!rabt");
