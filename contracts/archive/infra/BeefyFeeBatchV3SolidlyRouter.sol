...
contract RabbityieldFeeBatchV3SolidlyRouter is Initializable, OwnableUpgradeable {
    ...
    ISolidlyRouter.Routes[] public wNativeToRabtRoute;
    ...
    function initialize(
        ...
        address _rewardPool, 
        address _unirouter,
        ISolidlyRouter.Routes[] memory _rabtRoute,
        ...
        for (uint i; i < _rabtRoute.length; ++i) {
                wNativeToRabtRoute.push(_rabtRoute[i]);
            }
    ...
    function harvest() public {
        ...
        if (splitTreasury) {
            ...
            ISolidlyRouter(unirouter).swapExactTokensForTokens(treasuryHalf, 0, wNativeToRabtRoute, treasury, block.timestamp);
        } else {
    ...
    function setRoute(ISolidlyRouter.Routes[] memory _route, bool _stableRoute) external onlyOwner {
        ...
        if(_stableRoute) {
            ...
            delete wNativeToRabtRoute;
            require(_route[_route.length - 1].to == address(rabt), "!RABT");
             for (uint i; i < _route.length; ++i) {
                wNativeToRabtRoute.push(_route[i]);
            }
        }
    }
