...
import "./RabbityieldBalancerStructs.sol";
...
contract StrategyAuraGyroMainnet is StratFeeManagerInitializable {
    ...
    RabbityieldBalancerStructs.Input public input;
    ...
    RabbityieldBalancerStructs.BatchSwapStruct[] public nativeToLp0Route;
    RabbityieldBalancerStructs.BatchSwapStruct[] public lp0ToLp1Route;
    RabbityieldBalancerStructs.BatchSwapStruct[] public outputToNativeRoute;
    address[] public nativeToLp0Assets;
    address[] public lp0Tolp1Assets;
    address[] public outputToNativeAssets;

    mapping(address => RabbityieldBalancerStructs.Reward) public rewards;
    ...
    event ChargedFees(uint256 indexed callFees, uint256 indexed rabbityieldFees, uint256 indexed strategistFees);

    function initialize(
        address _want,
        RabbityieldBalancerStructs.BatchSwapStruct[] memory _nativeToLp0Route,
        RabbityieldBalancerStructs.BatchSwapStruct[] memory _lp0ToLp1Route,
        RabbityieldBalancerStructs.BatchSwapStruct[] memory _outputToNativeRoute,
        address _booster,
        uint256 _pid,
        ...
        for (uint i; i < rewardTokens.length; ++i) {
            ...
            if (bal >= rewards[rewardTokens[i]].minAmount) {
                if (rewards[rewardTokens[i]].assets[0] != address(0)) {
                    RabbityieldBalancerStructs.BatchSwapStruct[] memory swapInfo = new RabbityieldBalancerStructs.BatchSwapStruct[](rewards[rewardTokens[i]].assets.length - 1);
    ...
    function addRewardToken(address _token, RabbityieldBalancerStructs.BatchSwapStruct[] memory _swapInfo, address[] memory _assets, bytes calldata _routeToNative, uint _minAmount) external onlyOwner {
