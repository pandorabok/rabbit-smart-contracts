...
import "../../interfaces/rabbityield/IRabbityieldRewardPool.sol";
...
contract StrategyNum is StratFeeManagerInitializable {
    ...
    function chargeFees(address callFeeRecipient, uint256 _amount) internal returns (uint256 skim) {
        ...
        skim = _amount - fee;
        IRabbityieldSwapper(unirouter).swap(want, native, fee);
    ...
    function _notifyReward(uint256 _amount) internal {
        IRabbityieldSwapper(unirouter).swap(want, reward, _amount);
        uint256 rewardBal = IERC20(reward).balanceOf(address(this)) / duration * duration;
        if (rewardBal > 0) IRabbityieldRewardPool(rewardPool).notifyRewardAmount(reward, rewardBal, duration);
    }
