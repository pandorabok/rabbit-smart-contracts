...
import "../../interfaces/moe/IMoeChef.sol";
import "../Common/StratFeeManagerInitializable.sol";

contract StrategyMoe is StratFeeManagerInitializable {
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IMoeChef(chef).deposit(poolId, wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IMoeChef(chef).withdraw(poolId, _amount - wantBal);
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IMoeChef(chef).deposit(poolId, 0);
        _swapRewards();
    ...
    function _swapRewards() internal {
        ...
        for (uint i; i < rewardsLength; i++) {
            uint256 bal = IERC20(rewards[i][0]).balanceOf(address(this));
            if (bal > 0) {
                IUniswapRouterETH(unirouter).swapExactTokensForTokens(
                    bal, 0, rewards[i], address(this), block.timestamp
                );
            }
        }
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        return IMoeChef(chef).getDeposit(poolId, address(this));
    }
    ...
    function rewardsAvailable() public view returns (uint256) {
        ...
        pids[0] = poolId;
        (uint256[] memory moeRewards,,) = IMoeChef(chef).getPendingRewards(address(this), pids);
        return moeRewards[0];
    }
    ...
    function retireStrat() external {
        ...
        IMoeChef(chef).emergencyWithdraw(poolId);
    ...
    function _giveAllowances() internal {
        ...
        if (rewards.length != 0) {
            for (uint i; i < rewards.length; i++) {
                IERC20(rewards[i][0]).safeApprove(unirouter, 0);
                IERC20(rewards[i][0]).safeApprove(unirouter, type(uint).max);
            }
        }
    }
    ...
    function setReward(address[] calldata _rewardRoute) external onlyManager {
        address reward = _rewardRoute[0];
        require(reward != want, "reward != want");
        require(_rewardRoute[_rewardRoute.length - 1] == output, "end != output");

        IERC20(_rewardRoute[0]).safeApprove(unirouter, 0);
        IERC20(_rewardRoute[0]).safeApprove(unirouter, type(uint).max);

        rewards.push(_rewardRoute);
    }

    function resetRewards() external onlyManager {
        for (uint i; i < rewards.length; i++) {
            address reward = rewards[i][0];
