...
contract StrategySilo is StratFeeManagerInitializable {
    ...
    function _swapRewardsToNative() internal {
        for (uint i; i < rewards.length; ++i) {
            ...
            if (bal > 0 && reward != native) {
                IRabbityieldSwapper(unirouter).swap(reward, native, bal);
            }
        }
    }
    ...
    function _swapToWant() internal {
        uint256 bal = IERC20(native).balanceOf(address(this));
        if (want != native) {
            IRabbityieldSwapper(unirouter).swap(native, want, bal);
        }
    }
    ...
    function setReward(address _reward) external onlyManager {
        ...
        if (!paused()) IERC20(_reward).safeApprove(unirouter, type(uint).max);
