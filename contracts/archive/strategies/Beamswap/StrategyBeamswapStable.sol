...
contract StrategySolarbeamStable is StratManager, FeeManager {
    ...
    function chargeFees(address callFeeRecipient) internal {
        if (rewardToOutputRoute.length != 0) {
            for (uint i; i < rewardToOutputRoute.length; i++) {
                ...
                if (rewardBal > 0) {
                    IUniswapRouter(unirouter).swapExactTokensForTokens(rewardBal, 0, rewardToOutputRoute[i], address(this), now);
                }
            }
        }
    ...
    function addLiquidity() internal {
        uint256 outputBal = IERC20(output).balanceOf(address(this));
        IUniswapRouter(unirouter).swapExactTokensForTokens(outputBal, 0, outputToInputRoute, address(this), now);
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeBal;
        try IUniswapRouter(unirouter).getAmountsOut(rewardBal[0], outputToNativeRoute)
            ...
            for (uint i; i < rewardToOutputRoute.length; i++) {
                for (uint j = 1; j < rewardAdd.length; j++) {
                    if (rewardAdd[j] == rewardToOutputRoute[i][0]) {
                        try IUniswapRouter(unirouter).getAmountsOut(rewardBal[j], rewardToOutputRoute[i])
