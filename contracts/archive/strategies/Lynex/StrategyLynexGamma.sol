...
contract StrategyLynexGamma is StratFeeManagerInitializable {
    ...
    function hook(address sender, uint256 amount0, uint256, bytes memory) external {
        ...
        IRabbityieldSwapper(unirouter).swap(output, paymentToken, outputTokenBal);
        ...
        IRabbityieldSwapper(unirouter).swap(paymentToken, native, paymentTokenBal);
        flashOn = false;
    }
    ...
    function flashExercise(uint256 _amount) internal {
        ...
        flashOn = true;
        IFlashPool(flashPool).swap(amountNeeded, 0, address(this), "Rabbityield");
    }
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != native) IRabbityieldSwapper(unirouter).swap(native, lpToken0, toLp0);
        if (lpToken1 != native) IRabbityieldSwapper(unirouter).swap(native, lpToken1, toLp1);
    ...
    function quoteAddLiquidity() internal view returns (uint toLp0, uint toLp1) {
        ...
        if (isFastQuote) {
            ...
            uint out0 = lp0Amt;
            uint out1 = lp1Amt;
            if (lpToken0 != native) out0 = IRabbityieldSwapper(unirouter).getAmountOut(native, lpToken0, lp0Amt);
            if (lpToken1 != native) out1 = IRabbityieldSwapper(unirouter).getAmountOut(native, lpToken1, lp1Amt);
