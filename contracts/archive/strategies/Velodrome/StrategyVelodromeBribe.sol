...
import "../../interfaces/velodrome-v2/IReward.sol";
...
contract StrategyVelodromeBribe is StratFeeManagerInitializable {
    ...
    ISolidlyRouter.Route[] public outputToBribeRoute;
    ...
    function initialize(
        ...
        ISolidlyRouter.Route[] calldata _outputToBribeRoute,
        ...
        bribeRecipient = IVoter(voter).gaugeToBribe(gauge);

        for (uint i; i < _outputToNativeRoute.length; ++i) {
            outputToNativeRoute.push(_outputToNativeRoute[i]);
        }

        for (uint i; i < _outputToBribeRoute.length; ++i) {
            outputToBribeRoute.push(_outputToBribeRoute[i]);
        }
        ...
        bribe = outputToBribeRoute[outputToBribeRoute.length - 1].to;
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 rabbityieldFeeAmount = nativeBal * fees.rabbityield / DIVISOR;
    ...
    function bribeGauge() internal {
        uint256 bribeAmount = IERC20(output).balanceOf(address(this)) * bribeSplit / DIVISOR;
        if (bribeAmount > 0) {
            if (bribe != output) {
                bribeAmount = ISolidlyRouter(unirouter).swapExactTokensForTokens(
                    bribeAmount, 0, outputToBribeRoute, address(this), block.timestamp
                )[outputToBribeRoute.length];
            }

            IReward(bribeRecipient).notifyRewardAmount(bribe, bribeAmount);
        }
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(bribe).safeApprove(bribeRecipient, type(uint).max);
    ...
    function _removeAllowances() internal {
        ...
        IERC20(bribe).safeApprove(bribeRecipient, 0);

        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, 0);
    }

    function setBribe(ISolidlyRouter.Route[] memory _route) external onlyOwner {
        address source = _route[0].from;
        require(source == output, "!output");

        IERC20(bribe).safeApprove(bribeRecipient, 0);
        bribe = _route[_route.length - 1].to;
        if (!paused()) IERC20(bribe).safeApprove(bribeRecipient, type(uint).max);

        delete outputToBribeRoute;
        for (uint i; i < _route.length; ++i) {
            outputToBribeRoute.push(_route[i]);
        }
    }

    function setBribeSplit(uint256 _split) external onlyManager {
        require(_split < DIVISOR, "split>DIVISOR");
        bribeSplit = _split;
    }
    ...
    function outputToBribe() external view returns (address[] memory) {
        ISolidlyRouter.Route[] memory _route = outputToBribeRoute;
        return _solidlyToRoute(_route);
    }
