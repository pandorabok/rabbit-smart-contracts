...
contract BeTokenBatch is Ownable {
    ...
    address public rabbityieldFeeBatch;
    ...
    constructor(
        address _rewardPool,
        address _unirouter, 
        address _rabbityieldFeeBatch,
        address[] memory _route
    ) {
        rewardPool = IRewardPool(_rewardPool);
        unirouter = IUniswapRouterETH(_unirouter);
        rabbityieldFeeBatch = _rabbityieldFeeBatch;
        route = _route;
    ...
    function harvest() public {
        ...
        if (fee > 0) {
            ...
            unirouter.swapExactTokensForTokens(feeBal, 0, route, rabbityieldFeeBatch, block.timestamp);
        }
    ...
    function setFeeBatch(address _feeBatch) external onlyOwner {
        emit NewFeeBatch(rabbityieldFeeBatch, _feeBatch);
        rabbityieldFeeBatch = _feeBatch;
    }
