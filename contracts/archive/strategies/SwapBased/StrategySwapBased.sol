...
contract StrategySwapBased is StratFeeManagerInitializable {
    ...
    function _convertRewards() internal {
        ...
        if (rewards.length != 0) {
            for (uint i; i < rewards.length; i++) {
                ...
                if (toNative > 0) IRabbityieldSwapper(unirouter).swap(reward, native, toNative);
            }
        }
        ...
        if (optionBal > 0) {
            ...
            uint256 twapRate = IRabbityieldSwapper(unirouter).getAmountOut(output, native, optionBal);
    ...
    function receiveFlashLoan(
        ...
        IRabbityieldSwapper(unirouter).swap(output, native, IERC20(output).balanceOf(address(this)));
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != native) IRabbityieldSwapper(unirouter).swap(native, lpToken0, nativeHalf);
        if (lpToken1 != native) IRabbityieldSwapper(unirouter).swap(native, lpToken1, nativeHalf);
