...
contract StrategistBuyback is OwnableUpgradeable {
    ...
    function initialize(
        ...
        // approve spending by rabtMaxi
        IERC20_StrategistBuyback(native).approve(rabtMaxi, type(uint256).max);
        IERC20_StrategistBuyback(want).approve(rabtMaxi, type(uint256).max);

        trackVault(address(0)); // dummy vault to overcome issue where mapping values are defaulted to 0;
    }

    function depositVaultWantIntoRabtMaxi() external onlyOwner {
        _depositVaultWantIntoRabtMaxi();
    }

    function withdrawVaultWantFromRabtMaxi() external onlyOwner {
        _withdrawVaultWantFromRabtMaxi();
    }

    // Convert and send to rabbityield maxi
    ...
    function _depositVaultWantIntoRabtMaxi() internal {
        IVault_StrategistBuyback(rabtMaxi).depositAll();
    }

    function _withdrawVaultWantFromRabtMaxi() internal {
        IVault_StrategistBuyback(rabtMaxi).withdrawAll();
    }
    ...
    function balanceOfMooTokens() public view returns (uint256) {
        return IERC20_StrategistBuyback(rabtMaxi).balanceOf(address(this));
    }
