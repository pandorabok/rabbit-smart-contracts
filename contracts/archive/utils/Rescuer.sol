...
// Built to safely flash loan amount needed to upgrade Rabbityield Vault to new strategy 
contract Rescuer is Ownable {
    ...
    IRabbityieldVault public rabbityieldVault = IRabbityieldVault(0x42ECfA11Db08FB3Bb0AAf722857be56FA8E57Dc0);
    ...
    function tusdNeeded() public view returns (uint256) {
        return rabbityieldVault.balance() - tusd.balanceOf(address(iToken));
    }
    ...
    function upgrade() external onlyOwner {
        ...
        IStrategy(rabbityieldVault.strategy()).updateBalance();
        ...
        balancerVault.flashLoan(address(this), tokens, amounts, "RabbityieldBalancerFlash");
    }
    ...
    function receiveFlashLoan(address[] memory, uint256[] memory amounts, uint256[] memory feeAmounts, bytes memory) external {
        tusdPair.swap(0, tusdNeeded() - amounts[0], address(this), "RabbityieldRescue");
        uint256 balancerDebt = amounts[0] + feeAmounts[0];
        tusd.safeTransfer(address(balancerVault), balancerDebt);
    }

    function transferRabbityieldVaultOwnership(address _newOwner) external onlyOwner { 
        emit VaultOwnershipChanged(rabbityieldVault.owner(), _newOwner);
        rabbityieldVault.transferOwnership(_newOwner);
    }
