...
import "../interfaces/IRabbityieldVault.sol";
import "../interfaces/IRabbityieldStrategy.sol";
import "../interfaces/IRabbityieldRegistry.sol";
import "../interfaces/IRabbityieldHarvester.sol";
import "../interfaces/IUpkeepRefunder.sol";

import "../libraries/UpkeepLibrary.sol";

contract RabbityieldHarvester is ManageableUpgradeable, IRabbityieldHarvester {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // Contracts.
    IRabbityieldRegistry public _vaultRegistry;
    ...
    uint256 public _vaultHarvestFunctionGasOverhead; // Estimated average gas cost of calling harvest(). TODO: this needs to live in RabbityieldRegistry, and needs to be a `per vault` number.
    ...
    function initialize(
        ...
        _vaultRegistry = IRabbityieldRegistry(vaultRegistry_);
        ...
        )
    {
        ...
        if (numberOfVaultsToHarvest == 0) return (false, bytes("RabbityieldAutoHarvester: No vaults to harvest"));
    ...
    function _canHarvestVault(address vaultAddress_) internal view returns (bool canHarvest_) {
        IRabbityieldVault vault = IRabbityieldVault(vaultAddress_);
        IRabbityieldStrategy strategy = IRabbityieldStrategy(vault.strategy());
        ...
        )
    {
        ...
        uint256 vaultHarvestGasOverhead = _estimateSingleVaultHarvestGasOverhead(_vaultHarvestFunctionGasOverhead); // TODO: Pull this number from RabbityieldRegistry.
    ...
    function _harvestVault(address vault_) internal returns (bool didHarvest_, uint256 callRewards_) {
        IRabbityieldStrategy strategy = IRabbityieldStrategy(IRabbityieldVault(vault_).strategy());
