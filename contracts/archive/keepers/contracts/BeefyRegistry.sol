...
contract RabbityieldRegistry is ManageableUpgradeable, IRabbityieldRegistry {
    ...
    function _addVault(address _vaultAddress) internal {
        ...
        IRabbityieldVault vault = IRabbityieldVault(_vaultAddress);
        IRabbityieldStrategy strat = _validateVault(vault);
    ...
    function _validateVault(IRabbityieldVault _vault) internal view returns (IRabbityieldStrategy strategy) {
        address vaultAddress = address(_vault);

        try _vault.strategy() returns (IRabbityieldStrategy _strategy) {
            require(IRabbityieldStrategy(_strategy).vault() == vaultAddress, "Vault/Strat Mismatch");
            return IRabbityieldStrategy(_strategy);
        } catch {
            require(false, "Address not a Vault");
        }
    }

    function _collectTokenData(IRabbityieldStrategy _strategy) internal view returns (address[] memory tokens) {
    ...
    function getVaultInfo(address _vaultAddress)
        ...
        returns (
            string memory name_,
            IRabbityieldStrategy strategy_,
            bool isPaused_,
        ...
        )
    {
        ...
        name_ = vault.name();
        strategy_ = IRabbityieldStrategy(vault.strategy());
    ...
    function getStakedVaultsForAddress(address _address) external view returns (VaultInfo[] memory stakedVaults) {
        ...
        for (uint256 vid; vid < _vaultSet.length(); vid++) {
            if (IRabbityieldVault(_vaultSet.at(vid)).balanceOf(_address) > 0) {
                numResults++;
            }
        }
