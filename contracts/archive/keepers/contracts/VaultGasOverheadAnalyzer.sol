...
import "../interfaces/IRabbityieldStrategyEthCall.sol";
...
contract VaultGasOverheadAnalyzer is ManageableUpgradeable, KeeperCompatibleInterface {
    ...
    IRabbityieldRegistry private _vaultRegistry;
        ...
        )
    {
        ...
        IRabbityieldVault vaultToAnalyze = IRabbityieldVault(vaults[currentIndex]);
    ...
    function _analyzeHarvest(IRabbityieldVault vault) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
        IRabbityieldStrategyEthCall strategy = IRabbityieldStrategyEthCall(address(IRabbityieldVault(vault).strategy()));
    ...
    function _tryHarvest(IRabbityieldStrategyEthCall strategy) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
    ...
    function _tryOldHarvest(IRabbityieldStrategyEthCall strategy) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
        uint256 gasBefore = gasleft();
