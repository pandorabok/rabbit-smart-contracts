...
interface IRabbityieldVault is IERC20 {
    function deposit(uint256 amount) external;
    function withdraw(uint256 shares) external;
    function want() external pure returns (address); // Rabbityield Vault V6
    function token() external pure returns (address); // Rabbityield Vault V5
}

contract RabbityieldZapMdex {
    ...
    function beefOutAndSwap(address rabbityieldVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        ...
        require(token0 == desiredToken || token1 == desiredToken, 'Rabbityield: desired token not present in liquidity pair');
    ...
    function _getVaultPair (address rabbityieldVault) private view returns (IRabbityieldVault vault, IUniswapV2Pair pair) {
    ...
    function _swapAndStake(address rabbityieldVault, uint256 tokenAmountOutMin, address tokenIn) private {
        ...
        require(isInputA || pair.token1() == tokenIn, 'Rabbityield: Input token not present in liquidity pair');
