...
import {IRabbityieldVault} from "./zapInterfaces/IRabbityieldVault.sol";
import {IWETH} from "./zapInterfaces/IWETH.sol";
import {Babylonian} from "./libs/Babylonian.sol";

contract RabbityieldVelodromeV2Zap {
    ...
    constructor(address _router, address _WETH) {
        ...
        require(WETH == router.weth(), 'Rabbityield: WETH address not matching Router.weth()');
    }
    ...
    function beefOut (address rabbityieldVault, uint256 withdrawAmount) external {
        (IRabbityieldVault vault, IPool pair) = _getVaultPair(rabbityieldVault);
    ...
    function _getVaultPair (address rabbityieldVault) private pure returns (IRabbityieldVault vault, IPool pair) {
    ...
    function _swapAndStake(address rabbityieldVault, uint256 tokenAmountOutMin, address tokenIn) private {
        (IRabbityieldVault vault, IPool pair) = _getVaultPair(rabbityieldVault);
        require(pair.factory() == router.defaultFactory(), 'Rabbityield: Incompatible liquidity pair'); // router.addLiquidity adds to pair from router.defaultFactory()
    ...
    function estimateSwap(address rabbityieldVault, address tokenIn, uint256 fullInvestmentIn) public view returns (uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        (, IPool pair) = _getVaultPair(rabbityieldVault);
        require(pair.factory() == router.defaultFactory(), 'Rabbityield: Incompatible liquidity pair'); // router.addLiquidity adds to pair from router.defaultFactory()
