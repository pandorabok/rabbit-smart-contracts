...
// @author Wivern & Weso for Rabbityield.Finance
...
contract RabbityieldZapSolidlyDyst {
    ...
    function beefOut(address rabbityieldVault, uint256 withdrawAmount) external {
        (IRabbityieldVaultV6 vault, ISolidlyPair pair) = _getVaultPair(rabbityieldVault);
    ...
    function _getVaultPair(address rabbityieldVault) private pure returns (IRabbityieldVaultV6 vault, ISolidlyPair pair) {
    ...
    function estimateSwap(address rabbityieldVault, address tokenIn, uint256 fullInvestmentIn) public view returns(uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        checkWETH();
        (, ISolidlyPair pair) = _getVaultPair(rabbityieldVault);
