...
import "./zapInterfaces/IRabbityieldVault.sol";
import "./zapInterfaces/IStrategy.sol";
import "./zapInterfaces/IERC20Extended.sol";


// Aggregator Zap compatible with all single asset, uniswapv2, and solidly router Rabbityield Vaults. 
contract RabbityieldZapOneInch {
    ...
    function beefInETH (address _rabbityieldVault, bytes calldata _token0, bytes calldata _token1, WantType _type) external payable {
        require(msg.value >= minimumAmount, 'Rabbityield: Insignificant input amount');

        IWETH(WETH).deposit{value: msg.value}();
        _swapAndStake(_rabbityieldVault, WETH, WETH, _token0, _token1, _type);
        emit ZapIn(_rabbityieldVault, WETH, msg.value);
    }

    function beefIn (address _rabbityieldVault, address _inputToken, uint256 _tokenInAmount, bytes calldata _token0, bytes calldata _token1, WantType _type) public {
        require(_tokenInAmount >= minimumAmount, 'Rabbityield: Insignificant input amount');

        IERC20(_inputToken).safeTransferFrom(msg.sender, address(this), _tokenInAmount);
        _swapAndStake(_rabbityieldVault, _inputToken, _inputToken, _token0, _token1, _type);
        emit ZapIn(_rabbityieldVault,  _inputToken, _tokenInAmount);
    }

    function beefOut (address _rabbityieldVault, uint256 _withdrawAmount) external {
        address[] memory tokens = _beefOut(_rabbityieldVault, _withdrawAmount);
         _returnAssets(tokens);
    }

    function beefOutAndSwap(address _rabbityieldVault, uint256 _withdrawAmount, address _desiredToken, bytes calldata _dataToken0, bytes calldata _dataToken1, WantType _type) external {
        (IRabbityieldVault vault, IUniswapV2Pair pair) =  _getVaultPair(_rabbityieldVault);
        vault.safeTransferFrom(msg.sender, address(this), _withdrawAmount);
        vault.withdraw(_withdrawAmount);
         emit ZapOut(_rabbityieldVault, _desiredToken, _withdrawAmount);
    ...
    function beefOutAndReInvest(
        ...
        (IRabbityieldVault vault, IUniswapV2Pair pair) = _getVaultPair(_fromMooVault);
    ...
    function quoteStableAddLiquidityRatio(address _rabbityieldVault) external view returns (uint256 ratio1to0) {
            (IRabbityieldVault vault, IUniswapV2Pair pairAddress) = _getVaultPair(_rabbityieldVault);
    ...
    // Internal functions
    function _beefOut (address _rabbityieldVault, uint256 _withdrawAmount) private returns (address[] memory tokens) {
        (IRabbityieldVault vault, IUniswapV2Pair pair) = _getVaultPair(_rabbityieldVault);

        IERC20(_rabbityieldVault).safeTransferFrom(msg.sender, address(this), _withdrawAmount);
        vault.withdraw(_withdrawAmount);
        ...
        emit ZapOut(_rabbityieldVault, address(pair), _withdrawAmount);
    }
    ...
    function _getVaultPair (address _rabbityieldVault) private pure returns (IRabbityieldVault vault, IUniswapV2Pair pair) {
        vault = IRabbityieldVault(_rabbityieldVault);
    ...
    function _swapAndStake(address _vault, address _inputToken, bytes calldata _token0) private {
        IRabbityieldVault vault = IRabbityieldVault(_vault);
        address[] memory path;
        path = new address[](2);
        path[0] = vault.want();
        path[1] = _inputToken;
    ...
    function _swapAndStake(address _rabbityieldVault, address _inputToken0, address _inputToken1, bytes calldata _token0, bytes calldata _token1, WantType _type) private {
        (IRabbityieldVault vault, IUniswapV2Pair pair) =  _getVaultPair(_rabbityieldVault);
        ...
        if (_type != WantType.WANT_TYPE_SINGLE) {
            ...
            _swapAndStake(_rabbityieldVault, _inputToken0, _token0);
        }
    }
