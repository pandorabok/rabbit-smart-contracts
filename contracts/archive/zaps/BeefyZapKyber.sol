...
contract RabbityieldZapKyber {
    ...
    function beefInETH (address rabbityieldVault, bytes memory token0, bytes memory token1) external payable {
        require(msg.value >= minimumAmount, 'Rabbityield: Insignificant input amount');

        IWETH(WETH).deposit{value: msg.value}();
        _swapAndStake(rabbityieldVault, WETH, WETH, token0, token1);
    }

    function beefIn (address rabbityieldVault, address inputToken, uint256 tokenInAmount, bytes memory token0, bytes memory token1) public {
        require(tokenInAmount >= minimumAmount, 'Rabbityield: Insignificant input amount');
        require(IERC20(inputToken).allowance(msg.sender, address(this)) >= tokenInAmount, 'Rabbityield: Input token is not approved');

        IERC20(inputToken).safeTransferFrom(msg.sender, address(this), tokenInAmount);

        _swapAndStake(rabbityieldVault, inputToken, inputToken, token0, token1);
    }

    function beefOut (address rabbityieldVault, uint256 withdrawAmount) external {
        address[] memory tokens = _beefOut(rabbityieldVault, withdrawAmount);
         _returnAssets(tokens);
    }

    function _beefOut (address rabbityieldVault, uint256 withdrawAmount) internal returns (address[] memory tokens) {
        (IRabbityieldVault vault, IUniswapV2Pair pair) = _getVaultPair(rabbityieldVault);
    ...
    function beefOutAndSwap(address rabbityieldVault, uint256 withdrawAmount, address desiredToken, bytes memory dataToken0, bytes memory dataToken1) external {
        (IRabbityieldVault vault, IUniswapV2Pair pair) = _getVaultPair(rabbityieldVault);
    ...
    function quoteStableAddLiquidityRatio(IRabbityieldVault rabbityieldVault) external view returns (uint256 ratio0to1) {
            (,IUniswapV2Pair pairAddress) = _getVaultPair(address(rabbityieldVault));
            ...
            ISolidlyRouter router = ISolidlyRouter(IStrategy(rabbityieldVault.strategy()).unirouter());
    ...
    function quoteRemoveLiquidity(IRabbityieldVault rabbityieldVault, uint256 mooTokenAmt) external view returns (uint256 amt0, uint256 amt1, address token0, address token1) {
        uint256 withdrawFee = IStrategy(rabbityieldVault.strategy()).withdrawalFee();
        uint256 liquidity = mooTokenAmt * rabbityieldVault.balance() / rabbityieldVault.totalSupply();
        uint256 fee = withdrawFee > 0 ? liquidity * withdrawFee / 10000 : 0;
        liquidity = liquidity - fee;
        
        (, IUniswapV2Pair pair) = _getVaultPair(address(rabbityieldVault));
    ...
    function _getVaultPair (address rabbityieldVault) private pure returns (IRabbityieldVault vault, IUniswapV2Pair pair) {
        vault = IRabbityieldVault(rabbityieldVault);
    ...
    function _swapAndStake(address rabbityieldVault, address inputToken0, address inputToken1, bytes memory token0, bytes memory token1) private {
        (IRabbityieldVault vault, IUniswapV2Pair pair) = _getVaultPair(rabbityieldVault);
