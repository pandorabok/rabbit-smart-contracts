...
task("test-data:addressbook:rabbityield", "Fetch rabbityield addressbook to inject platform addresses in forge tests")
  ...
  .setAction(async (taskArgs: { chain: keyof typeof addressBook}, hre, runSuper) => {
    ...
    const { rabbityieldfinance } = addressBook[taskArgs.chain].platforms;
    const data = {
      keeper: rabbityieldfinance.keeper,
      strategyOwner: rabbityieldfinance.strategyOwner,
      vaultOwner: rabbityieldfinance.vaultOwner,
    };
